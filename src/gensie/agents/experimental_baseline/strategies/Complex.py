...
class Complex(StrategyV2):
    ...
    def execute(self, in_time=0)-> Dict[str, Any]:
        ...
        if self.max_time > self.estimated_time:
            print(f"Complex: using slow method with estimated_time={self.estimated_time} (max_time={self.max_time})")
            result = self.baseline_fit_to_fields(reasoning_effort="high")
        else:
            print(f"Complex: using fast method with estimated_time={self.estimated_time} (max_time={self.max_time})")
            result = self.baseline_fit_to_fields(reasoning_effort="low")
        self.exe_time = time.time() - init
        return result

    def slow_method(self):
                # 1. tag text
        hints, total_tokens, exe_time = self.llm.get_candidates(self.task, self.plain_schema, self.task.input_text, return_usage=True, reasoning_effort="high")
        ...
        result, total_tokens, exe_time = self.llm.direct_call(self.task, use_schema=True,
                                      seed=43, temperature=0.5, user_prompt=user_prompt,
                                      system_name="complex_system", return_usage=True,
    ...
    def fast_method(self):
                # 1. tag text
        hints, total_tokens, exe_time = self.llm.get_candidates(self.task, self.plain_schema, self.task.input_text, return_usage=True)
        self.tokens.append(total_tokens)
        self.times.append(exe_time)
