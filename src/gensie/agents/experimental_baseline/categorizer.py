...
def fit_schema_to_fields_dict(fields: list[str], schema) -> dict:
    """Return a simplified schema dict containing only the given fields, for use in LLM prompts."""
    defs = schema.get("$defs", {})
    props = schema.get("properties", {})
    new_props = {field: props[field] for field in fields if field in props}
    return {
        "type": "object",
